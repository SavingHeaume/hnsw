...
public:
  ...
  size_t max_elements_{0};                          // 索引可容纳的最大元素数量
  mutable std::atomic<size_t> cur_element_count{0}; // 当前索引中的元素数量
  mutable std::atomic<size_t> num_deleted_{0};      // 已删除元素的数量

  // 内存布局相关
  size_t size_data_per_element_{0}; // 每个元素在第0层占用的总内存大小
  size_t size_links_per_element_{
      0}; // 每个元素在高层（>0层）的链接列表占用的内存大小
  size_t size_links_level0_{0}; // 第0层链接列表占用的内存大小
  size_t offsetData_{0}, offsetLevel0_{0},
      label_offset_{0}; // 数据在内存块中的偏移量
  // 第0层链接列表的偏移量
  // 标签在内存块中的偏移量
  size_t data_size_{0}; // 单个数据向量的大小

  // hnsw参数
  size_t M_{0};               // 每个节点在第1层及以上的最大连接数
  size_t maxM_{0};            // 等于 M_，第1层及以上每层的最大连接数
  size_t maxM0_{0};           // 第0层的最大连接数，通常为 M_ * 2
  size_t ef_construction_{0}; // 构建索引时的动态候选列表大小
  size_t ef_{0};              // 搜索时的动态候选列表大小

  // 层级相关
  double mult_{0.0}, revSize_{0.0}; // 层级选择的倍数因子，值为 1/ln(M),
                                    // revSize是_mult_ 的倒数，即 ln(M)
  int maxlevel_{0};                 // 当前索引中的最大层级
  std::vector<int> element_levels_; // 存储每个元素所在的层级
  ...
  // 内存存储
  char *data_level0_memory_{nullptr}; // 存储所有元素第0层数据的连续内存块
  char **linkLists_{nullptr}; // 指针数组，存储每个元素高层（>0层）的链接列表

  // 距离计算
  DISTFUNC<dist_t> fstdistfunc_;   // 距离计算函数指针
  void *dist_func_param_{nullptr}; // 距离函数的参数
  mutable std::atomic<long> metric_distance_computations{
      0};                                   // 距离计算次数的统计
  mutable std::atomic<long> metric_hops{0}; // 跳转次数的统计
  ...
  // 比较器，通过只比较距离值来管理搜索过程中的候选节点优先级队列
  ...
  searchBaseLayer(tableint ep_id, const void *data_point, int layer) {
    ...
    std::priority_queue<std::pair<dist_t, tableint>,
                        ...
                        CompareByFirst>
        top_candidates; // 存储当前找到的最佳结果（正距离，大的在堆顶）
    std::priority_queue<std::pair<dist_t, tableint>,
                        std::vector<std::pair<dist_t, tableint>>,
                        CompareByFirst>
        candidateSet; // 存储待探索的候选节点（负距离，小的在堆顶）
